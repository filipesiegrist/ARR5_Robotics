...
def enviar_coordenadas(entry_j1, entry_j2, entry_j3):
	...
	ser.write(b'2 ')
	ser.write(str(entry_y).encode('ascii'))
	sleep(2)

	ser.write(b'3 ')
	ser.write(str(entry_z).encode('ascii'))
	sleep(2)

	#print("A coordenada eh: [%s, %s, %s]" % (entry_j1, entry_j2, entry_j3))
...
button_send = tk.Button(frame_xyz, text="Enviar\n\nCoord", font=40, command=lambda: enviar_coordenadas(entry_j1.get(), entry_j2.get(), entry_j3.get()))
